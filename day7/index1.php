...
div.desc
{
  ...
  <body>
    ...
    <header class="clearfix">
      ...
      <div>
        <h1>Welcome</h1>
        <div class="fb-login-button" data-scope="friends_likes,user_likes,user_photos"></div>
      </div>
      <?php } ?>
    </header>
	...
	$count=0;
			$arr1=array();
	foreach($friends as $new){
		$count=$count+1;// just a count to see how many times it's entering this loop
		$id = idx($new, 'id');
		$name = idx($new, 'name');
	     // print_r($new);
		$likesthis = idx($facebook->api('/'.$id.'/movies'), 'data', array());
		if($id == $user_id) // the user's movie names are stored in a different array
					{
									continue;
											}
		foreach ($likesthis as $like) {
			// Extract the pieces of info we need from the requests above
			$id = idx($like, 'id');
			$item = idx($like, 'name');
		
			if(!array_key_exists($id1,$arr_user) and !array_key_exists($id,$arr1) ){
				$arr1[$id]=$item;
			...
			?>
				<div class="img">
				<a href="https://www.facebook.com/<?php echo he($id); ?>" target="_top">
				<img src="https://graph.facebook.com/<?php echo he($id) ?>/picture?type=large" alt="<?php echo he($item); ?>" width="110" height="90">
				<?//php echo he($item);	?>
				</a>  <div class="desc"><?php   $re="php genrating.php ".str_replace(" ","-",$arr1[$id]); /*system($re);*/ echo $arr1[$id]; ?></div> 
				</div> 
				</div>
				<?php
				}
